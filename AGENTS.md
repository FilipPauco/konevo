...
- At the start of every task, read and apply `.github/copilot-instructions.md` as additional project instructions. Treat it as shared prompt context for GitHub Copilot and Codex.
- If these instructions conflict with `.github/copilot-instructions.md`, follow the more specific rule for the current file or task and mention the conflict before proceeding.
...
- Out of the box, `core_components.ex` provides a `<.icon name="icon-[tabler--x]" class="w-5 h-5"/>` component backed by the `@iconify/tailwind4` plugin. **Always** use the `<.icon>` component for icons
...
- For UI components, follow this priority: 1. existing codebase component → 2. FlyonUI component ([flyonui.com/docs](https://flyonui.com/docs)) → 3. custom Tailwind component
...
LiveViews that require login should **always be placed inside the __existing__ `live_session :require_authenticated_user` block**:
    ...
    scope "/", AppWeb do
      ...
      live_session :require_authenticated_user,
        on_mount: [{KonevoWeb.UserAuth, :require_authenticated}] do
...
LiveViews that can work with or without authentication, **always use the __existing__ `:current_user` scope**, ie:
    ...
    scope "/", MyAppWeb do
      ...
      live_session :current_user,
        on_mount: [{KonevoWeb.UserAuth, :mount_current_scope}] do
