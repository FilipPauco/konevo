defmodule KonevoWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, KonevoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      KonevoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "silently redirects home if there isn't a valid user_token", %{conn: conn} do
      user_token = "invalid_token"
      session = conn |> put_session(:user_token, user_token) |> get_session()
    ...
    test "silently redirects home if there isn't a user_token", %{conn: conn} do
      session = conn |> get_session()
  ...
  describe "require_authenticated_user/2" do
    ...
    test "silently redirects home if user is not authenticated", %{conn: conn} do
    ...
    test "does not retain the requested path", %{conn: conn} do
      halted_conn =
      ...
      assert redirected_to(halted_conn) == ~p"/"
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        KonevoWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
